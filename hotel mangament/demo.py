...
def add_db():
    print(rand.get())
    try:
         if rand.get() != "" and cost.get() != "" and Service_Charge.get() != "" and Tax.get() != ""  and Subtotal.get() != "" and Total.get()!= "":
            print(rand.get())
            con = sqlite3.connect(database=r'Cafe.db')
            cur = con.cursor()
            print(rand.get())
            cur.execute("Select Order_No from Order_Info where Order_No=? ",(rand.get(),))
            print(rand.get())
            row = cur.fetchone()
            print(row)
            if row == None:
                print(rand.get())
                cur.execute("insert into Order_Info values(?,?,?,?,?,?)",(
                            rand.get(),
                            cost.get(),
                            Service_Charge.get(),
                            Tax.get(),
                ...
                cur.close()
                con.close()
                messagebox.showinfo("Success","Data Added Successfuly")
            else:
            
               messagebox.showwarning("waring",f"Order No is already Present in database")
            
         else:
              messagebox.showwarning("waring",f"All Fields Are Required")
                     
    
    except Exception as es:
        messagebox.showwarning("waring",f"Some went wrong:{str(es)}")
...
add=Button(f1,padx=16,pady=8, bd=10 ,fg="black",font=('ariel' ,16,'bold'),width=10, text="Add", bg="#65dd00",command=add_db)
add.grid(row=9, column=0,pady=10)

# ==============================show data Table=====================================

Order2_table=Frame(root,bd=2,relief=RIDGE)
Order2_table.place(x=900,y=200,width=600,height=350)
Scroll_x=ttk.Scrollbar(Order2_table,orient=HORIZONTAL)
Scroll_y=ttk.Scrollbar(Order2_table,orient=VERTICAL)
Order_info_table=ttk.Treeview(Order2_table,column=("Order_No","Cost","Service_charge","Tax","Subtotal","Total"),xscrollcommand=Scroll_x.set,yscrollcommand=Scroll_y.set)

Scroll_x.pack(side=BOTTOM,fill=X)
Scroll_y.pack(side=RIGHT,fill=Y)

Scroll_x.config(command=Order_info_table.xview)
Scroll_y.config(command=Order_info_table.yview)
